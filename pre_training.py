...
parser.add_argument(
    "--experiment_id", type=str, help="Experiment name (MNIST,CIFAR,CelebA)."
)
parser.add_argument(
    "--model_dir",
    type=str,
...
assert args.experiment_id in [
    "MNIST",
    "CIFAR",
    "CelebA",
], "experiment_id must be one of MNIST, CIFAR, CelebA"
...
CONF = yaml.load(
    open(os.path.join(args.config_dir, "pretrain_conf_" + args.experiment_id + ".yml")),
...
if args.experiment_id == "MNIST":
    ...
    train_dataset = CIFAR100(
        CONF["DataRoot"], download=False, train=True, transform=transform_train
    )
    val_dataset = CIFAR100(
        CONF["DataRoot"], download=False, train=False, transform=transform_val
    )
    ...
    train_dataset = CelebA(
        CONF["DataRoot"], download=False, split="train", transform=transform
    )
    val_dataset = CelebA(
        CONF["DataRoot"], download=False, split="valid", transform=transform
    )
...
if args.experiment_id in ["CIFAR", "CelebA"]:
    ...
    train_loader = DataLoader(
        train_dataset, batch_size=CONF["BATCH_SIZE1"], shuffle=True
    )

    val_loader = DataLoader(val_dataset, batch_size=CONF["BATCH_SIZE1"], shuffle=True)
...
if args.experiment_id == "CIFAR":
    ...
    opt1 = torch.optim.Adam(
        ...
        lr=CONF["LR1"],
        weight_decay=CONF["WEIGHT_DECAY1"],
    )
    ...
    opt1 = torch.optim.Adam(
        classifier.fc.parameters(), lr=CONF["LR1"], weight_decay=CONF["WEIGHT_DECAY1"]
    )
    summary(classifier, input_size=(3, 224, 224))
    ...
    for epoch in range(n_epochs):
        ...
        for data in train_loader:
            ...
            if args.experiment_id == "CIFAR":
                ...
                running_acc += [
                    100 * (predicted == labels).sum().item() / (len(labels) * 40)
                ]
            ...
            if i % display_step == display_step - 1:
                ...
                print(
                    f"[{epoch + 1}, {i + 1}] loss: {train_loss}, acc: {train_acc}",
                    flush=True,
                )
...
train_loader = DataLoader(train_dataset, batch_size=CONF["BATCH_SIZE2"], shuffle=True)

val_loader = DataLoader(val_dataset, batch_size=CONF["BATCH_SIZE2"], shuffle=True)
    ...
    if epoch % 10 == 9:
        if args.experiment_id == "MNIST":
            ...
            torch.save(
                classifier.state_dict(),
                args.model_dir
                + "/WideResNet50_CelebA_Attributes_"
                + str(epoch + 1)
                + "EP.pth",
            )
...
if args.experiment_id == "MNIST":
    torch.save(
        classifier.state_dict(),
        args.model_dir + "/VGGtiny_classifier_EMNIST_" + str(epoch + 1) + "EP.pth",
    )
elif args.experiment_id == "CIFAR":
    torch.save(
        classifier.state_dict(),
        args.model_dir + "/EfficientNetB0_CIFAR100_" + str(epoch + 1) + "EP.pth",
    )
elif args.experiment_id == "CelebA":
    torch.save(
        classifier.state_dict(),
        args.model_dir + "/WideResNet50_CelebA_Attributes_" + str(epoch + 1) + "EP.pth",
    )

...
parser.add_argument(
    "--binary", type=str, default="F", help="Training binary classifiers? Answer T/F"
)
parser.add_argument(
    "--config_dir",
    type=str,
...
if args.data_id in ["CIFAR10", "CIFAR100"]:
    if flag_binary:
        CONF = yaml.load(
            open(os.path.join(args.config_dir, "shadow_conf_binary_CIFAR.yml")),
    ...
    CONF = yaml.load(
        open(os.path.join(args.config_dir, "shadow_conf_CelebA.yml")),
        Loader=yaml.FullLoader,
    )
elif flag_binary:
    CONF = yaml.load(
        open(
            os.path.join(args.config_dir, "shadow_conf_binary_" + args.data_id + ".yml")
        ),
        Loader=yaml.FullLoader,
    )
else:
...
assert (CONF["BATCH_SIZE"] == "Full") or type(
    CONF["BATCH_SIZE"]
) == int, "BATCH_SIZE must be Full or an integer"
...
if args.data_id in ["MNIST", "CIFAR10", "CIFAR100"]:
    ...
    for training_no in range(models_per_seed):
        ...
        weights_flat = torch.cat(
            [p.data.flatten() for p in head.parameters()], 0
        ).numpy()
    ...
    while len(trained_models_weights) < models_per_seed:
        ...
        if torch.sum(mask) > class_size:
            train_ind = torch.nonzero(mask).reshape(
                -1,
            )
            ...
            rand_inds_train = torch.randint(
                high=len(features_comp), size=(9 * class_size, 1)
            )
            ...
            Y_train = (
                torch.cat([torch.ones(class_size), torch.zeros(9 * class_size)], 0)
                .to(torch.float32)
                .reshape(-1, 1)
            )
            ...
            Y_val = (
                torch.cat([torch.ones(nV), torch.zeros(9 * nV)], 0)
                .to(torch.float32)
                .reshape(-1, 1)
            )
            ...
            while not flag_stop:
                ...
                acc_metric.reset()
                print(
                    "Attempt ", attempts + 1, " final loss: ", loss.item(), flush=True
                )
                attempts += 1

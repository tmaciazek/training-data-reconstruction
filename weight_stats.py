...
parser.add_argument(
    "--filename_header", type=str, help="The header of the weight data filenames."
)
parser.add_argument(
...
parser.add_argument(
    ...
    help="Directory for saving the weight stats.",
)
...
np.save(
    args.stats_dir + "/" + "stats_mean_" + args.filename_header + ".npy",
    w_mean.cpu().numpy(),
)
np.save(
    args.stats_dir + "/" + "stats_std_" + args.filename_header + ".npy",
    w_std.cpu().numpy(),
)
...
if torch.sum(w_std == 0.0) > 0:
    print(
        "Vanishing variance! Adding jitter terms to the covariance matrix...",
        flush=True,
    )
    jitter = 1e-6
    w_std = w_std + jitter
    w_cov = w_cov + jitter * np.eye(w_cov.shape[0]).to(device)

w_cov = torch.divide(w_cov, w_std.T @ w_std)
np.save(
    args.stats_dir + "/" + "stats_covnorm_" + args.filename_header + ".npy",
    w_cov.cpu().numpy(),
)

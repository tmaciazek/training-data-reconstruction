...
parser.add_argument(
    "--data_id", type=str, help="Dtaset name (MNIST, CIFAR10, CIFAR100, CelebA)."
)
...
assert args.data_id in [
    ...
    "CIFAR100",
    "CelebA",
], "data_id must be one of MNIST, CIFAR10, CIFAR100, CelebA"
...
if args.data_id in ["CIFAR10", "CIFAR100"]:
    CONF = yaml.load(
        open(os.path.join(args.config_dir, "pretrain_conf_CIFAR.yml")),
        Loader=yaml.FullLoader,
    )
else:
    CONF = yaml.load(
        open(os.path.join(args.config_dir, "pretrain_conf_" + args.data_id + ".yml")),
        Loader=yaml.FullLoader,
    )
...
if args.data_id == "MNIST":
    ...
    train_dataset = MNIST(
        CONF["DataRoot"], download=False, train=True, transform=transform
    )
    val_dataset = MNIST(
        CONF["DataRoot"], download=False, train=False, transform=transform
    )
    ...
    print(
        "No. classes, train data size, val data size:",
        n_classes,
        n_train_data,
        n_val_data,
        flush=True,
    ...
    classifier.load_state_dict(
        torch.load(args.model_dir + "/" + args.model, map_location=device)
    )
    ...
    top = nn.Sequential(
        nn.Linear(in_features=2048, out_features=40, bias=True), nn.Sigmoid()
    )
    top = top.to(device)
    classifier.fc = top
    classifier.load_state_dict(
...
np.save(
    args.features_dir + "/" + args.data_id + "_features_train.npy",
    features_train.cpu().numpy(),
)
np.save(
    args.features_dir + "/" + args.data_id + "_labels_train.npy",
    labels_train.cpu().numpy(),
)
np.save(
    args.features_dir + "/" + args.data_id + "_features_val.npy",
    features_val.cpu().numpy(),
)
np.save(
    args.features_dir + "/" + args.data_id + "_labels_val.npy", labels_val.cpu().numpy()
)
...
if args.data_id == "CelebA":
    ...
    train_dataset = CelebA(
        ".", download=False, split="train", target_type="identity", transform=transform
    )
    val_dataset = CelebA(
        ".", download=False, split="valid", target_type="identity", transform=transform
    )
    train_dataset = ConcatDataset([train_dataset, val_dataset])
    val_dataset = CelebA(
        ".", download=False, split="test", target_type="identity", transform=transform
    )
    ...
    np.save(
        args.celeba_img_dir + "/CelebA_img_64x64_train.npy", img_train.cpu().numpy()
    )

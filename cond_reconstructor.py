...
def set_softmin_mse_mae_lpips(outputs, sets, alpha=100.0):
    ...
    out_repeat = outputs.repeat_interleave(CL, dim=1).reshape(-1, CH, RES, RES)
    lpips_batch = lpips_loss_fn.forward(out_repeat, sets.reshape(-1, CH, RES, RES))
    lpips_table = lpips_batch.reshape(B, CL)
...
def mse_mae_lpips(outputs, sets):
    ...
    out_repeat = outputs.repeat_interleave(CL, dim=1).reshape(-1, CH, 32, 32)
    lpips_batch = lpips_loss_fn.forward(out_repeat, sets.reshape(-1, CH, 32, 32))
    lpips_table = lpips_batch.reshape(B, CL)
...
def set_min_mse(fake_img, real_sets):
    mse = torch.mean(torch.square(fake_img - real_sets), dim=(2, 3, 4))
    return torch.mean(torch.min(mse, dim=1)[0])
...
weights_val_data, images_val_data = load_rec_data(
...
weights_data, images_data = load_rec_data(
    img_data=img_data,
...
generator_input_dim = weights_data.shape[1] + 10
out_channels = images_data.shape[-3]
rec = Reconstructor(
    z_dim=generator_input_dim, gen_size=CONF["GEN_SIZE"], out_channels=out_channels
).to(device)
...
# prepare the validation data by appending one-hot labels
weights_val_data = weights_val_data.unsqueeze(1)
weights_val_data = torch.repeat_interleave(weights_val_data, 10, dim=1)
images_val_data = images_val_data
one_hot_labels = torch.eye(10).unsqueeze(0)
one_hot_labels = torch.repeat_interleave(one_hot_labels, len(weights_val_data), dim=0)
weights_val_one_hot = torch.cat([weights_val_data, one_hot_labels], -1)
weights_val_one_hot = weights_val_one_hot.reshape(-1, generator_input_dim)
val_batch_size = CONF["VAL_BATCH_SIZE"]
val_nb = int(len(weights_val_one_hot) / float(val_batch_size))
...
for epoch in range(CONF["TRAINING_EPOCHS"]):
    ...
    for weights, real in dataloader:
        ...
        rand_class = torch.randint(low=0, high=10, size=(cur_batch_size,)).to(device)
        rand_class_one_hot = F.one_hot(rand_class, num_classes=10)
        weights = torch.cat([weights, rand_class_one_hot], 1).to(device)
        random_real_image = [
            torch.unsqueeze(real_b[inds_b], 0)
            for real_b, inds_b in zip(real, rand_class)
        ]
        random_real_image = torch.cat(random_real_image, 0).to(device)
        ...
        gen_loss = rec_loss_fn(fake, random_real_image)
    ...
    with torch.no_grad():
        for b in range(val_nb + 1):
            weights_val_batch = weights_val_one_hot[
                b
                * val_batch_size : min(
                    (b + 1) * val_batch_size, len(weights_val_one_hot)
                )
            ]
